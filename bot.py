...
# Cấu hình bot
BOT_TOKEN = "8330490898:AAGdH3HayTNQjiuNEqNeIJDvEm1o4UNsrwA"  # Thay thế bằng token bot của bạn
ADMIN_IDS = [8114375011]  # Danh sách ID admin
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        await update.message.reply_text("❌ *Bạn không có quyền sử dụng bot này*", parse_mode='Markdown')
        return
    ...
    if message_text.startswith('+') or message_text.startswith('-'):
        try:
            ...
            if amount < 0 and abs(amount) > group_data["totalAmount"]:
                await update.message.reply_text("⚠️ *Lỗi:* Không đủ tiền để trừ!", parse_mode='Markdown')
                return
...
✅ *GIAO DỊCH ĐÃ ĐƯỢC XỬ LÝ THÀNH CÔNG!* ✅"""
            ...
            await update.message.reply_text("⚠️ *Lỗi:* Vui lòng nhập số hợp lệ sau dấu + hoặc -", parse_mode='Markdown')
        except Exception as e:
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
        ...
        await update.message.reply_text(f"❌ *Lỗi khi đọc dữ liệu:* {str(e)}", parse_mode='Markdown')

# Xử lý lệnh /reset_time
async def reset_time_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    chat_id = update.effective_chat.id
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Kiểm tra quyền admin
        if user_id not in ADMIN_IDS:
            await query.edit_message_text("❌ *Bạn không có quyền sử dụng bot này*", parse_mode='Markdown')
            return
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
    await update.message.reply_text(welcome_message, parse_mode='Markdown', reply_markup=create_admin_keyboard(chat_id))
...
def main():
    ...
    bot_application = application  # Lưu để auto reset có thể sử dụng
    
    # Thêm các handler
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("summary", summary_command))
    application.add_handler(CommandHandler("test", lambda u, c: u.message.reply_text("✅ Bot đang hoạt động!")))
    application.add_handler(CommandHandler("reset_time", reset_time_command))
